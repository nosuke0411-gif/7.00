...
def calculate_mines_multiplier(opened, bombs):
    ...
    return round(multiplier * 1.15, 3)  # 本家調整
...
class MinesButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view = MinesView(self.user_id)
        for i in range(25):
            view.add_item(MinesButton(i, self.user_id))
        view.add_item(MinesStopButton(self.user_id))
...
class MinesStopButton(discord.ui.Button):
    def __init__(self, user_id):
        super().__init__(label="ストップ", style=discord.ButtonStyle.danger, row=5)
        self.user_id = user_id
...
@app_commands.describe(bet="賭けるコイン", bombs="爆弾の数（1〜24）")
async def mine(interaction: discord.Interaction, bet: int, bombs: int):
    ...
    if bombs < 1 or bombs >= 25:
        await interaction.followup.send("⚠️ 爆弾の数は 1〜24 にしてね！")
        return
    ...
    # 爆弾配置
    bomb_positions = set(random.sample(range(25), bombs))
    ...
    for i in range(25):
        view.add_item(MinesButton(i, user_id))
    view.add_item(MinesStopButton(user_id))
